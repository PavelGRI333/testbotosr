...
values. It is used by :class:`bot.services.gemini_service.GeminiService`.
"""

INVOICE_SYSTEM_PROMPT = """Ты специалист по обработке накладных и товарных документов. Проанализируй изображение и извлеки данные. Не придумывай значения. Если данные отсутствуют, используй null. Возвращай только валидный JSON без какой‑либо разметки (без markdown, без ```json, первый символ должен быть '{').
